...
- Best crops per district: Nyagatare=Maize/Sorghum, Musanze=Potato/Wheat, Karongi/Rubavu=Rice/Beans, Huye=mixed, Kirehe=Rice/Beans, Kigali=all
...
Response style:
- Simple, clear English (greet warmly in Kinyarwanda: Muraho, Murakoze)
- Practical and actionable with specific RWF price ranges
    ...
    if request.method == 'POST':
        ...
        if bundle and crop in crops and district in districts:
            ...
            pred_obj = PricePrediction.objects.create(
                user=request.user, crop=crop, district=district,
                season=season, month=month, year=year,
                predicted_price=price, advice=advice_text,
            )
...
def export_prediction_pdf(request, pk):
    ...
    thresholds = {
        'Maize':{'sell_now':380,'hold':300},'Beans':{'sell_now':950,'hold':750},
        'Irish Potato':{'sell_now':300,'hold':200},'Wheat':{'sell_now':500,'hold':400},
        'Sorghum':{'sell_now':350,'hold':260},'Rice':{'sell_now':1000,'hold':850},
    }
    ...
    if pred.predicted_price >= t['sell_now']:
        ...
        advice_color = "#d97706"
    html = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>RAB Price Prediction Report</title>
<style>
  ...
  .advice {{ padding:14px 18px; border-radius:10px; font-weight:600; margin-bottom:20px; border-left:5px solid {advice_color}; background:{advice_color}18; color:{advice_color} }}
  ...
  .print-btn {{ background:#16a34a; color:white; border:none; padding:12px 28px; border-radius:8px; font-size:15px; font-weight:bold; cursor:pointer; margin-bottom:24px }}
</style></head><body>
<button class="print-btn no-print" onclick="window.print()">Print / Save as PDF</button>
<div class="header"><h1>RAB Crop Price Prediction Report</h1>
<p>Rwanda Agriculture and Animal Resources Development Board - Generated {datetime.date.today().strftime("%d %B %Y")}</p></div>
<div class="price-box">
...
<div class="footer">RAB Smart Farming Platform - Rwanda Agriculture Board - Rubona, Huye District<br>
Toll free: 4675 - info@rab.gov.rw - Generated by ML model (R2=98%)</div>
</body></html>"""
...
def agent_chat(request):
    try:
        ...
        # FIXED: dedicated system_instruction (stronger AI persona)
        payload = json.dumps({
            "system_instruction": {
        ...
        # FIXED: gemini-1.5-flash is the correct valid free model name
        url = (
        ...
        code = e.code
        if code == 503:
            return JsonResponse({'reply': 'The AI is temporarily busy. Please wait 10 seconds and try again.'})
        elif code == 429:
            return JsonResponse({'reply': 'Too many requests sent. Please wait 30 seconds and try again.'})
        else:
            return JsonResponse({'reply': f'API error {code}. Please try again in a moment.'})
